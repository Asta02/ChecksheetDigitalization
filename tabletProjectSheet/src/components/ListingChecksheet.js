...
const ChecksheetTerbaru = ({ id, title, brand, type, customer, status, tanggal }) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const response = await fetch(`http://10.19.101.166:3003/getChecksheetTerbaru/${npk}`);
                if (!response.ok) {
...
export default ChecksheetTerbaru;
